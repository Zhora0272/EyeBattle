    ...
    public class UILayer : MonoBehaviour
    {
        ...
        internal void AddScreenInfo(ScreenInfo screenInfo)
        {
            ...
            if (screenInfo.Prefab == null)
            {
                GameLog.LogError(
                    "UIFrame", 
                    $"Layer {_layerInfo.Name} has a null reference.", 
                    this);
                return;
            }
            ...
            if (screen == null)
            {
                GameLog.LogError(
                    "UIFrame", 
                    $"UIScreenBase component not found on {screenInfo.Prefab.name} in layer {_layerInfo.Name}.",
                    this);
                return;
            }
            ...
            if (_screensByType.ContainsKey(screenType) || _screenInfosByType.ContainsKey(screenType))
            {
                GameLog.LogError(
                    "UIFrame", 
                    $"Screen {screenType.Name} already added to the layer!",
                    this);
                return;
            }
