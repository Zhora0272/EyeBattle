...
namespace VFavorites
{
    public static class VFavorites
    {
        ...
        static void UpdateOnGUIWrapping()
        {
            ...
            if (wrappedBrowser && (!curEvent.alt || !wrappedBrowserHovered) && !mouseLeftWhileDragging)
            {
                wrappedBrowser.Repaint();
                UnwrapBrowserOnGUI();
            }

            if (!wrappedBrowser && curEvent.alt && anyBrowserHovered)
            {
        ...
        [MenuItem("Tools/vFavorites/Enabled")] static void asdsasda() { enabled = !enabled; UnityEditor.Compilation.CompilationPipeline.RequestScriptCompilation(); }
        [MenuItem("Tools/vFavorites/Enabled", true)] static bool adsassad() { Menu.SetChecked("Tools/vFavorites/Enabled", enabled); return true; }

        static bool enabled { get => EditorPrefs.GetBool("vFavorites-enabled", true); set => EditorPrefs.SetBool("vFavorites-enabled", value); }
    }
}
#endif
